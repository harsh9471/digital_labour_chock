...
const STATUS_MAP: Record<string, { label: string; cls: string; border: string }> = {
  PLANNING:  { label: 'Planning',  cls: 'bg-blue-50 text-blue-700 border-blue-200',          border: 'border-l-4 border-l-blue-500' },
  ACTIVE:    { label: 'Active',    cls: 'bg-emerald-50 text-emerald-700 border-emerald-200',  border: 'border-l-4 border-l-emerald-500' },
  ON_HOLD:   { label: 'On Hold',   cls: 'bg-amber-50 text-amber-700 border-amber-200',        border: 'border-l-4 border-l-amber-500' },
  COMPLETED: { label: 'Completed', cls: 'bg-purple-50 text-purple-700 border-purple-200',     border: 'border-l-4 border-l-purple-500' },
  CANCELLED: { label: 'Cancelled', cls: 'bg-red-50 text-red-700 border-red-200',             border: 'border-l-4 border-l-red-400' },
};
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total',     value: total,           gradient: 'from-blue-600 to-blue-700',     shadow: 'shadow-blue-500/20' },
          { label: 'Active',    value: stats.active,    gradient: 'from-emerald-500 to-teal-600',  shadow: 'shadow-emerald-500/20' },
          { label: 'Planning',  value: stats.planning,  gradient: 'from-indigo-500 to-blue-600',   shadow: 'shadow-indigo-500/20' },
          { label: 'Completed', value: stats.completed, gradient: 'from-purple-500 to-violet-600', shadow: 'shadow-purple-500/20' },
        ].map(s => (
          ...
          <>
            <div className="divide-y divide-gray-50">
              {projects.map(project => (
                <div key={project.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-gray-50/80 transition-colors group ${STATUS_MAP[project.status]?.border ?? 'border-l-4 border-l-gray-200'}`}>
